...
export async function getHabitCardData(
  ...
  let completion_data_colRef = collection(
    habit_participation_ref,
    "completions",
  );
  ...
  let habitDoc: DocumentData = await getDoc(
    habit_participation_doc.data().habit_ref,
  );
  ...
  for (let i = 0; i < habit_participants.length; i++) {
    ...
    let participant_completion_data_colRef = collection(
      habit_participants[i],
      "completions",
    );
    ...
    const info = {
      created_at: participant_doc.data().created_at,
